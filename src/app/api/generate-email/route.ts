...
async function generateGeminiMessages({ recipientName, programs, organization }: {
  ...
  const prompt = `
    Generate THREE distinct message options to ${firstName} congratulating them on completing ${formattedPrograms}.
    ...
    2. Congratulate them on completing ${formattedPrograms}
    3. Mention that their certificate${programs.length > 1 ? "s are" : " is"} attached
...
Warmest congratulations on completing ${formattedPrograms}! Your certificate${programs.length > 1 ? "s are" : " is"} attached.
...
Congratulations on successfully completing ${formattedPrograms}. Your certificate${programs.length > 1 ? "s are" : " is"} attached.
...
Awesome job completing ${formattedPrograms}! Your certificate${programs.length > 1 ? "s are" : " is"} attached.
...
export async function POST(req: Request) {
  try {
    ...
    const messages = await generateGeminiMessages({ recipientName, programs, organization });
    return NextResponse.json({ messages });
  } catch (err) {
    console.error("generate-email route error:", err);
    
    try {
      // Return formatted fallback messages
      const { recipientName, programs } = await req.json();
      const firstName = recipientName.split(' ')[0];
